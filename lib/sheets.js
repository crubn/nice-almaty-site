...
async function fetchIgnoredFromSheet() {
  if (!SHEETS_ID) return [];
  const now = Date.now();
  // Use != null so a successful empty tab (`[]`) still caches, but null means "never fetched".
  if (ignoreCache.phones != null && now - ignoreCache.at < TTL_MS) return ignoreCache.phones;
