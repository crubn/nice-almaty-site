...
const TABS = {
  ...
  // Chats where a human manager is handling — bot stays silent.
  mute: process.env.SHEETS_TAB_MUTE || "Молчит бот",
};
...
async function fetchIgnoredFromSheet() {
  if (!SHEETS_ID) return [];
  const now = Date.now();
  // Empty tab uses a short TTL so newly added resident numbers pick up quickly.
  const ttl = (ignoreCache.phones && ignoreCache.phones.length) ? TTL_MS : Math.min(TTL_MS, 60_000);
  if (ignoreCache.phones != null && now - ignoreCache.at < ttl) return ignoreCache.phones;
  const csv = await fetchCsvSafe(TABS.ignore);
  // On fetch failure keep the previous list; don't cache a synthetic empty list.
  if (csv === null) {
    if (ignoreCache.phones != null) return ignoreCache.phones;
    return [];
  }
...
// Rows for manager-mute tab: { phone, until? }. Missing tab → [].
async function fetchMuteRows() {
  if (!SHEETS_ID) return [];
  const csv = await fetchCsvSafe(TABS.mute);
  if (csv === null) return null; // fetch failed — caller keeps previous cache
  const objs = toObjects(parseCSV(csv || ""));
  return objs.map((o) => ({
    phone: field(o, ["Телефон", "Phone", "Номер", "chatId", "ChatId"]),
    until: field(o, ["До", "Until", "until", "Expires"]),
  })).filter((r) => r.phone);
}
