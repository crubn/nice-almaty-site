...
function buildSystemPrompt(opts) {
  ...
  const lines = [
    ...
    "- You have NO access to residents' personal data (names, phone numbers). If asked who lives somewhere, politely refuse and say you can only share availability.",
    "- 'free' = available now, 'reserved' = booked (see note for the date), 'occupied' = taken. When counting free places for a student, count only 'free'.",
    "- If a house is full, proactively suggest houses that still have free places.",
  ...
  lines.push(
    "",
    "=== LIVE SEAT AVAILABILITY (generated " + ((availability && availability.generatedAt) || "n/a") + ", no personal data) ===",
    JSON.stringify((availability && availability.houses) || {}),
    "",
    "=== HOUSE FACTS (prices, districts, routes, amenities, contacts) ===",
