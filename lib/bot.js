...
function stripAiSlop(s) {
  ...
  // Only strip filler at the END of a paragraph (not mid-sentence FAQ content).
  const trailingSlop = /(?:[.!?]\s+|\n+)(?:если\s+у\s+вас\s+есть\s+вопрос[^.!?\n]{0,80}|я\s+с\s+радостью\s+помогу[^.!?\n]{0,60}|просто\s+напишите[^.!?\n]{0,80}|дайте\s+знать[^.!?\n]{0,60}|чем\s+ещё\s+могу\s+помочь[^.!?\n]{0,40}|feel\s+free\s+to\s+ask[^.!?\n]{0,40}|let\s+me\s+know\s+if\s+you\s+have[^.!?\n]{0,60})[.!?]?$/iu;
  // Pure fluff ONLY — opener + empty closer, no real FAQ body.
  const pureSlopOnly = /^(?:если\s+у\s+вас\s+есть\s+вопросы?[,\s]+(?:дайте\s+знать|просто\s+напишите|напишите|пишите|обращайтесь)[^.!?]{0,40}|чем\s+ещё\s+могу\s+помочь[^.!?]{0,40}|буду\s+рад[а]?\s+помочь[^.!?]{0,40}|с\s+радостью\s+помогу[^.!?]{0,40}|дайте\s+знать[^.!?]{0,40}|feel\s+free\s+to\s+ask[^.!?]{0,40}|let\s+me\s+know\s+if\s+you\s+have\s+(?:any\s+)?questions?[^.!?]{0,40})[.!?]?$/iu;

  // Drop trailing paragraphs that are pure filler (never wipe a sole useful paragraph).
  ...
  if (pureSlopOnly.test(t)) return "";
  t = t.replace(trailingSlop, (m) => (/^[.!?]\s/.test(m) ? m[0] : ""));
  t = t.replace(/[ \t]+\n/g, "\n").replace(/\n{3,}/g, "\n\n");
  t = t.replace(/\s+([,.!?;:])/g, "$1").replace(/[ \t]{2,}/g, " ").trim();
  t = t.replace(/\.\s*\./g, ".").replace(/[.!?]\s*[.!?]+$/g, (m) => m[0]).replace(/\n{3,}/g, "\n\n").trim();
  // Residue with no letters/digits is empty (avoids shipping a lone ".").
...
// Tight meta scrub — do NOT match ordinary housing phrases like «внутренние правила».
const META_LEAK_RE = /языков[а-яёa-z]*\s+модел|deepseek|gpt-?\d|баз[а-яё]*\s+данн|стек\s+технолог|внутренн[а-яёa-z]+\s+(?:промпт|модел)|(?:^|[^\p{L}])промпт(?:[^\p{L}]|$)|нейросет/iu;
