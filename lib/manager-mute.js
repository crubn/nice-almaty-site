 ...
 *
 * Still require authorName/authorId when present helps, BUT the decisive guard is
 * lib/bot-sends.js: we remember every Admin API send and never mute on our own echoes
 * (Wazzup sometimes marks them isEcho=true).
 */
function isPhoneOutbound(m, isGroupFn, opts) {
  opts = opts || {};
  if (!m || typeof m !== "object") return false;
  if (typeof isGroupFn === "function" && isGroupFn(m)) return false;
  if (isBotCrmMessage(m)) return false;
  if (typeof opts.isOurSend === "function" && opts.isOurSend(m)) return false;
  if (m.isEcho !== true) return false;
  return true;
}

/** Human manager on Phone — never Admin API. */
function isHumanOutbound(m, isGroupFn, opts) {
  return isPhoneOutbound(m, isGroupFn, opts);
}

function classifyOutbound(m, isGroupFn, opts) {
  opts = opts || {};
  if (!m || typeof m !== "object") return "unknown";
  if (typeof isGroupFn === "function" && isGroupFn(m)) return "group";
  if (typeof opts.isOurSend === "function" && opts.isOurSend(m)) return "admin_api";
  if (isBotCrmMessage(m) || isAdminApiOutbound(m)) return "admin_api";
  if (isPhoneOutbound(m, isGroupFn, opts)) return "phone";
...
function noteManagerActivity(messages, opts) {
  ...
  const isOurSend = opts.isOurSend; // (m, chatKey) => boolean
  // Mute ONLY when Phone writes (isEcho) and it is NOT one of our Admin API sends.
  const muted = [];
  for (const m of messages || []) {
    const id = typeof extractChatId === "function" ? extractChatId(m) : "";
    const key = id && typeof normalizePhone === "function" ? (normalizePhone(id) || id) : id;
    const our = typeof isOurSend === "function" ? ((msg) => isOurSend(msg, key)) : null;
    const kind = classifyOutbound(m, isGroupFn, our ? { isOurSend: our } : {});
